...
class BayesianLogisticRegression(LinearClassifierMixin, BaseEstimator):
    ...
    def __init__(self, n_iter, tol, fit_intercept, verbose):
        ...
        self._first_partial_fit_complete = False
    ...
    def partial_fit(self,X,y):
        return self._fit_model(X, y, self._first_partial_fit_complete)

    def _fit_model(self,X,y,partial=False):
        # preprocess data
        ...
        for i in range(len(self.coef_)):
            ...
            if partial:
                coef_, sigma_ = self._fit(X,y_bin, self.coef_[i],
                                          1/self.sigma_[i])
            else:
...
class StreamingLogisticRegression(BayesianLogisticRegression):
    '''
    Implements Streaming Bayesian Logistic Regression using a Gaussian (Laplace)
    approximation for the posterior
    ...
    def __init__(self, tol = 1e-3,solver = 'lbfgs_b', n_iter_solver = 15,
                 tol_solver = 1e-3, fit_intercept = True, alpha = 1e-6,
                 verbose = False):
        super(StreamingLogisticRegression,self).__init__(0, tol,
    ...
    def _fit(self,X,y,w0=None,prec=None):
        '''
        Calculate Laplace approximation
        '''
        n_samples, n_features = X.shape
        if prec is None:
            prec = self.alpha * np.ones(n_features)
        if w0 is None:
            w0 = np.zeros(n_features)
        # find mean & covariance of Laplace approximation to posterior
        coef_, sigma_ = self._posterior(X, y, prec, w0)
        self._first_partial_fit_complete = True
        return coef_, sigma_
    ...
    def _posterior(self, X, Y, prec0, w0):
        '''
        ...
        '''
        n_samples,n_features  = X.shape
        # prior_hess = sp.diags(prec0) if sp.issparse(X) else np.diag(prec0)
        def prior_loss(w):
            return .5 * np.dot((w - w0) * prec0, w - w0)
        prior_loss_grad = lambda w: (w - w0) * prec0
        #prior_loss_hess = lambda w: prior_hess
        ...
        #     f    = lambda w, X, Y, prec0: _logistic_loss(w, X, Y, 0) + prior_loss(w)
        #     grad = lambda w,*args: _logistic_loss_and_grad(w,*args)[1] + prior_loss_grad(w)
        #     hess = lambda *args: _logistic_grad_hess(*args) + prior_hess
        #     args = (X[:,:-1],Y,alpha0)
        ...
        np.fill_diagonal(Hess, np.diag(Hess) + prec0)
        # e  =  eigvalsh(Hess)
        return w, np.diag(np.linalg.inv(Hess))
